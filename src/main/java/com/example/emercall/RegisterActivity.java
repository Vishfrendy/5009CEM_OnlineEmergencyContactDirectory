    ...
    public void register(final View view) {
        ...
        firebaseAuth.createUserWithEmailAndPassword(email,password).addOnCompleteListener(task -> {
            if (task.isSuccessful()){
                ...
                documentReference.set(mapUser).addOnSuccessListener(aVoid -> Log.d("TAG","onSuccess: user Profile is created for "+ mapUser)).addOnFailureListener(new OnFailureListener() {
                    @Override
                ...
                user.sendEmailVerification().addOnSuccessListener(aVoid -> Snackbar.make(view,"Verification Email has been Sent", BaseTransientBottomBar.LENGTH_LONG).show()).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
